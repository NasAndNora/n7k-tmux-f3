...
class ClaudeMessage(Static):
    """Orange-themed message for Claude responses."""

    def __init__(self, content: str = "") -> None:
        super().__init__()
        self.add_class("claude-message")
        self._content = content
        self._markdown: Markdown | None = None
        self._stream: MarkdownStream | None = None

    def compose(self) -> ComposeResult:
        with Horizontal(classes="claude-message-container"):
            yield Static("● ", classes="claude-message-dot")
            with Vertical(classes="claude-message-content"):
                markdown = Markdown("")
                self._markdown = markdown
                yield markdown
    ...
    async def replace_content(self, content: str) -> None:
        """Replace entire content (for tmux polling where we get full content each time)."""
        if not content or content == self._content:
            return
        self._content = content
        # Reset markdown and rewrite
...
class GeminiMessage(Static):
    """Blue-themed message for Gemini responses using Markdown rendering."""

    def __init__(self, content: str = "") -> None:
        super().__init__()
        self.add_class("gemini-message")
        self._content = content
        self._markdown: Markdown | None = None
        self._stream: MarkdownStream | None = None

    def compose(self) -> ComposeResult:
        with Horizontal(classes="gemini-message-container"):
            yield Static("✦ ", classes="gemini-message-dot")
            with Vertical(classes="gemini-message-content"):
                markdown = Markdown("")
                self._markdown = markdown
                yield markdown
