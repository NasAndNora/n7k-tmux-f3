...
# F17: Moved to context_progress.py - displays in UI instead of terminal
# def check_dependencies() -> None:
#     """Check system dependencies and print warnings if missing."""
#     warnings = []
#
#     # tmux
#     if not shutil.which("tmux"):
#         warnings.append("tmux not found - required for AI sessions")
#
#     # Node.js >= 20
#     node = shutil.which("node")
#     if not node:
#         warnings.append("Node.js not found - required for MCP")
#     else:
#         try:
#             result = subprocess.run([node, "--version"], capture_output=True, text=True)
#             version = result.stdout.strip().lstrip("v").split(".")[0]
#             if int(version) < 20:
#                 warnings.append(f"Node.js {result.stdout.strip()} found - v20+ required")
#         except Exception:
#             pass
#
#     # claude CLI
...
#         warnings.append("gemini CLI not found")
#
#     # Print warnings
#     if warnings:
#         rprint("[yellow]⚠ Missing dependencies:[/]")
#         for w in warnings:
#             rprint(f"[yellow]  • {w}[/]")
#         rprint()
...
def main() -> None:
    # F17: check_dependencies() moved to UI (context_progress.py)
    load_api_keys_from_env()
