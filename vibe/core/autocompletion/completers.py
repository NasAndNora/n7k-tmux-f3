...
class AITagCompleter(Completer):
    """Completer for AI routing tags (@cc, @claude, @g, @gemini)."""
    ...
    def _extract_tag_partial(self, before_cursor: str) -> str | None:
        """Extract partial tag after @ symbol."""
        if "@" not in before_cursor:
            return None
        ...
        # Don't complete if there's a space (tag already finished)
        if " " in fragment:
            return None

        # Don't complete if it looks like a file path
    ...
    def get_completions(self, text: str, cursor_pos: int) -> list[str]:
        before_cursor = text[:cursor_pos]
        partial = self._extract_tag_partial(before_cursor)

        if partial is None:
            return []

        matches = []
        for tag, (full_tag, _) in self.AI_TAGS.items():
            if tag.startswith(partial):
                matches.append(full_tag)
    ...
    def get_completion_items(self, text: str, cursor_pos: int) -> list[tuple[str, str]]:
        ...
        matches = []
        for tag, (full_tag, description) in self.AI_TAGS.items():
            if tag.startswith(partial):
                matches.append((full_tag, description))
    ...
    def get_replacement_range(
        ...
        if "@" in before_cursor:
            at_idx = before_cursor.rfind("@")
            fragment = before_cursor[at_idx + 1 :]
            # Only replace if this looks like a tag, not a file path
            if "/" not in fragment and "." not in fragment:
                return (at_idx, cursor_pos)
        return None
