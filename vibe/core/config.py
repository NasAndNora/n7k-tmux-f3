...
class VibeConfig(BaseSettings):
    ...
    skip_startup_screen: bool = False  # F6: Skip StartupScreen in tests
    ...
    def _check_api_key(self) -> VibeConfig:
        # Skip API key check in debate mode (uses CLI backends, not API)
        if os.getenv("DEBATE_MODE"):
            return self
        try:
    ...
    def _migrate(cls) -> None:
        ...
        if (
            ...
            or config["auto_compact_threshold"] == 100_000
        ):
