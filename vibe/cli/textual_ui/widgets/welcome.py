...
class WelcomeBanner(Static):
    ...
    def __init__(self, config: VibeConfig) -> None:
        ...
        # F8: AI backend status (None=pending, "ok"=ready, other=error message)
        self._ai_status: dict[str, str | None] = {"claude": None, "gemini": None}
    ...
    def _initialize_static_line_suffixes(self) -> None:
        ...
        # F8: Line 3 = models count + AI status, Line 4 = MCP count
        self._static_line3_suffix = f"{self.LOGO_TEXT_GAP}[dim]{model_count} models ·[/] {self._build_ai_status_text()}"
        self._static_line4_suffix = (
            f"{self.LOGO_TEXT_GAP}[dim]{mcp_count} MCP servers[/]"
        )
    ...
    def _build_ai_status_text(self) -> str:
        """Build AI status text with colors: grey=pending, green=ok, red=failed."""
        parts = []
        for name, status in self._ai_status.items():
            display_name = name.capitalize()
            if status is None:
                parts.append(f"[dim]{display_name}[/]")
            elif status == "ok":
                parts.append(f"[green]✓ {display_name}[/]")
            else:
                parts.append(f"[red]✗ {display_name}[/]")
        return f"{' · '.join(parts)}"

    def update_ai_status(self, statuses: dict[str, str]) -> None:
        """Update AI backend status and refresh display (F8)."""
        for name, status in statuses.items():
            self._ai_status[name.lower()] = status
        # Rebuild line 3 suffix with new status
        model_count = len(self.config.models)
        self._static_line3_suffix = f"{self.LOGO_TEXT_GAP}[dim]{model_count} models ·[/] {self._build_ai_status_text()}"
        # Force refresh line 3 (index 2)
        if len(self._line_states) > 2:
            self._cached_text_lines[2] = Text.from_markup(
                self._build_line(2, self._get_color(2))
            )
    ...
    def _build_line(self, line_idx: int, color: str) -> str:
        ...
        patterns = [
            ...
            f"{S}[{color}]{B}[/]{S}[{color}]{B}[/]{S}[{color}]{B}[/]{S}{self._static_line4_suffix}",
