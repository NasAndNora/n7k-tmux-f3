...
def vibe_config() -> VibeConfig:
    return VibeConfig(
        session_logging=SessionLoggingConfig(enabled=False), skip_startup_screen=True
    )


@pytest.fixture
def vibe_app(vibe_config: VibeConfig) -> VibeApp:
    return VibeApp(config=vibe_config, debate_mode=False)
...
@pytest.mark.asyncio
async def test_path_completion_popup_shows_all_results(
    vibe_app: VibeApp, file_tree: Path
) -> None:
    async with vibe_app.run_test() as pilot:
